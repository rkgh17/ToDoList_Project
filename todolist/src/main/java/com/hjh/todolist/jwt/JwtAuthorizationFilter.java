...
/**
 * 지정한 URL별 JWT 유효성 검증을 수행하며 직접적인 사용자 '인증'을 확인합니다.
 * 
 * 해당 클래스는 Spring Security의 환경설정을 구성하는 단계에서 필터로 등록한 클래스이며, 
...
public class JwtAuthorizationFilter extends OncePerRequestFilter{
	...
	protected void doFilterInternal(HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain chain)
		...
		// 1. 토큰이 필요하지 않는 API URL에 대해 배열로 구성.
		List<String> list = Arrays.asList(
				"/api/login",
				"/api/signup"
				);
		
		// 2. 토큰이 필요하지 않는 API ULR의 경우 로직 처리 없이 다음 필터로 이동.
		if(list.contains(request.getRequestURI())) {
			chain.doFilter(request, response);
			return;
		}
		
		// 3. OPTIONS 요청을 경우 => 로직 처리 없이 다음 필터로 이동
		if(request.getMethod().equalsIgnoreCase("OPTIONS")) {
			chain.doFilter(request, response);
			return;
		}
		
		// [STEP1] Client에서 API를 요청할 때 Header를 확인합니다.
		String header = request.getHeader(AuthConstants.AUTH_HEADER);
		logger.debug("[+] header Check: " + header);
		
		try {
			// [STEP A] Header 내에 토큰이 존재하는 경우
